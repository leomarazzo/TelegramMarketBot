...
class MarketBot:
    ...
    def start_new_list(self, message):
        ...
        self.bot.send_message(message.chat.id, f"New list created for: {message.from_user.username}")
    ...
    def add_to_list(self, message):
        if message.chat.id in self.lists:
            ...
            self.bot.send_message(message.chat.id, f"No list created for: {message.from_user.username}.\nUse the /new command to create a new list")
